...
CONFIGSRC = "${RTKDIR}/yocto/poky/openembedded-core/meta/recipes-connectivity/wpa-supplicant/wpa-supplicant"
EXTERNALSRC = "${RTKDIR}/firmware/component/wifi/whc/whc_host_linux/wpa_supplicant"
EXTERNALSRC_BUILD = "${EXTERNALSRC}"

S = "${EXTERNALSRC}"

inherit pkgconfig systemd externalsrc
...
do_configure () {
	...
	sed -i \
          -e 's/^CONFIG_IEEE8021X_EAPOL=y/#CONFIG_IEEE8021X_EAPOL=n/' \
          -e 's/^CONFIG_EAP_TLS=y/#CONFIG_EAP_TLS=n/' \
          -e 's/^CONFIG_EAP_PEAP=y/#CONFIG_EAP_PEAP=n/' \
          -e 's/^CONFIG_EAP_TTLS=y/#CONFIG_EAP_TTLS=n/' \
...
do_install () {
	...
	install -m 644 wpa_supplicant/README ${CONFIGSRC}/wpa_supplicant.conf ${D}${docdir}/wpa_supplicant
	...
	install -m 755 ${CONFIGSRC}/wpa-supplicant.sh ${D}${sysconfdir}/network/if-pre-up.d/wpa-supplicant
	...
	install -m 0644 ${CONFIGSRC}/99_wpa_supplicant ${D}/etc/default/volatiles
...
python split_wpa_supplicant_libs () {
    ...
    split_packages = do_split_packages(
        d,
        libdir,
        r'^(.*)\.so',
        '${PN}-plugin-%s',
        'wpa_supplicant %s plugin',
        prepend=True
    )

    split_dbg_packages = do_split_packages(
        d,
        dbglibdir,
        r'^(.*)\.so',
        '${PN}-plugin-%s-dbg',
        'wpa_supplicant %s plugin - Debugging files',
        prepend=True,
